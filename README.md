...
2. **用户列表**：新增要统计的人（只需姓名）。
3. **服务器管理 → 添加服务器**：填地址、端口、SSH 用户名，录入/拖入私钥，选“归属用户”，确定即可。平台会自动：记录主机指纹（首次连接自动信任，之后指纹变化会拒绝连接）→ 检查采集组件、没有则经 SSH 自动安装 → 创建 Claude Code 与 Codex 两个采集目标 → 启动首次采集。某个工具的目录在远端不存在时显示“未使用”，不算失败。
4. 需要最小权限的服务器，可改用手工方式：在服务器上执行 `remote/install.sh "<对应公钥>"`（密钥被限制为只能运行采集脚本），再到平台上添加；两种方式的对比见 DEPLOY.md §3。
