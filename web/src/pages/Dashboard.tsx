...
export function DashboardPage() {
  ...
  const daily = useFetch(() => api.get<DailyRanking>('/stats/daily-ranking', { date: rankDate }), [rankDate], { keepPrevious: true });
  ...
  const todayTop = useMemo(() => {
    ...
    return todayAll.filter((r) => val(r, k.tokens) > 0 || val(r, k.cost) > 0)
  ...
  const rankDatePicker = (
    <DatePicker
      size={isMobile ? 'middle' : 'small'} allowClear={false} inputReadOnly={isMobile} value={shownDate ? dayjs(shownDate) : null} style={{ width: 130 }}
  ...
  const rankBySwitch = <Segmented size={isMobile ? 'middle' : 'small'} value={rankBy} onChange={(v) => setRankBy(v as RankBy)} options={RANK_OPTIONS} />;

  // 手机上的当日排名：只显示所选口径（合计 / Claude Code / Codex）的 Token 与费用两列，不需要横向滚动；切换“排名依据”即可看另一口径
  const rk = RANK_KEYS[rankBy];
  const rankLabel = RANK_OPTIONS.find((o) => o.value === rankBy)?.label ?? '';
  const dailyColumns: ColumnsType<TodayRow> = isMobile
    ? [
      { title: '#', width: 28, render: (_v, _r, i) => i + 1 },
      { title: '用户', render: (_v, r) => <div className="wrap-anywhere"><Link to={`/users/${r.userId}`}>{r.name}</Link><div><Typography.Text type="secondary" style={{ fontSize: 12 }}>{r.team ?? '未分组'}</Typography.Text></div></div> },
      { title: 'Token', align: 'right', render: (_v, r) => <SourceTokens value={r[rk.tokens] as TodayRow['totalTokens']} /> },
      { title: '估算费用', align: 'right', render: (_v, r) => <Typography.Text strong><SourceCost value={r[rk.cost] as number | null} tokens={r[rk.tokens] as TodayRow['totalTokens']} /></Typography.Text> },
    ]
    : [
  ...
  return (
    <>
      ...
      <Card
        title={`${isToday ? '今日' : shownDate + ' '}用量排名 Top 10`} size="small" style={{ marginTop: gap }}
        extra={isMobile ? undefined : <Space size={8} wrap><Typography.Text type="secondary">日期</Typography.Text>{rankDatePicker}<Typography.Text type="secondary">排名依据</Typography.Text>{rankBySwitch}</Space>}
      >
        {isMobile && (
          <div style={{ marginBottom: 12 }}>
            <FilterBar items={[{ key: 'date', node: rankDatePicker }, { key: 'by', node: rankBySwitch }]} />
          </div>
        )}
        {daily.error && <Alert type="error" showIcon title={daily.error} style={{ marginBottom: 12 }} action={<Button size="small" onClick={daily.reload}>重试</Button>} />}
        <Table<TodayRow>
          size="small" rowKey="userId" pagination={false} dataSource={todayTop} loading={daily.loading} scroll={isMobile ? undefined : { x: 820 }}
          locale={{ emptyText: isToday ? '今日还没有用量' : '这一天没有用量记录' }}
          columns={dailyColumns}
          summary={() => (todayAll.length === 0 ? null : isMobile ? (
            <Table.Summary.Row style={{ background: '#fafafa' }}>
              <Table.Summary.Cell index={0} colSpan={2}><Typography.Text strong>全员{rankBy === 'total' ? '合计' : ` ${rankLabel}`}（{todayAll.length} 人）</Typography.Text></Table.Summary.Cell>
              <Table.Summary.Cell index={2} align="right"><SourceTokens value={todaySum[rk.tokens as keyof typeof todaySum]} /></Table.Summary.Cell>
              <Table.Summary.Cell index={3} align="right"><Typography.Text strong><SourceCost value={todaySum[rk.cost as keyof typeof todaySum]} tokens={todaySum[rk.tokens as keyof typeof todaySum]} /></Typography.Text></Table.Summary.Cell>
            </Table.Summary.Row>
          ) : (
            <Table.Summary.Row style={{ background: '#fafafa' }}>
      ...
      <Row gutter={gutter} style={{ marginTop: gap }}>
        <Col xs={24} xl={14}>
          <Card title="本月用量排名 Top 10" size="small">
            {/* 手机：团队、今日 Token 两列隐藏（responsive），预算使用率并到用户名下面，四列正好放下 */}
            <Table
              size="small" rowKey="userId" pagination={false} dataSource={data?.ranking ?? []} loading={first} scroll={isMobile ? undefined : { x: 640 }}
              columns={[
                { title: '#', width: isMobile ? 28 : 40, render: (_v, _r, i) => i + 1 },
                { title: '用户', render: (_v, r) => (isMobile
                  ? <div className="wrap-anywhere"><Link to={`/users/${r.userId}`}>{r.name}</Link><div style={{ maxWidth: 150 }}><BudgetBar compact cost={r.monthCost} budget={r.monthlyBudgetUsd} /></div></div>
                  : <Link to={`/users/${r.userId}`}>{r.name}</Link>) },
                { title: '团队', dataIndex: 'team', responsive: ['md'], render: (v: string | null) => v ?? <Typography.Text type="secondary">未分组</Typography.Text> },
                { title: '今日 Token', align: 'right', responsive: ['md'], render: (_v, r) => <TokenCell value={r.todayTokens} /> },
                { title: '本月 Token', align: 'right', render: (_v, r) => <TokenCell value={r.monthTokens} /> },
                { title: isMobile ? '本月费用' : '本月估算费用', align: 'right', render: (_v, r) => <CostCell value={r.monthCost} /> },
                { title: '预算使用率', width: 200, responsive: ['md'], render: (_v, r) => <BudgetBar cost={r.monthCost} budget={r.monthlyBudgetUsd} /> },
              ]}
            />
          </Card>
        </Col>
        <Col xs={24} xl={10}>
          <Card title="采集异常" size="small" extra={<Link to="/servers">服务器管理</Link>}>
            <Table
              size="small" rowKey="targetId" pagination={false} dataSource={data?.issues ?? []} loading={first} scroll={isMobile ? undefined : { x: 480 }}
              locale={{ emptyText: '所有来源均已按期更新' }}
              columns={[
                {
                  // 手机：最近成功时间与错误详情并到来源下面直接写出来（桌面端的错误详情靠悬停查看，触屏上够不着）
                  title: '来源', render: (_v, r) => (
                    <div className={isMobile ? 'wrap-anywhere' : undefined}>
                      <div>{r.serverName}</div>
                      <Typography.Text type="secondary" style={{ fontSize: 12 }}>{r.dataDir} · {r.userName}</Typography.Text>
                      {isMobile && <div><Typography.Text type="secondary" style={{ fontSize: 12 }}>最近成功 {fmtTime(r.lastSuccessAt)}</Typography.Text></div>}
                      {isMobile && r.lastErrorCode && <div><Typography.Text type="danger" style={{ fontSize: 12 }}>{r.lastErrorCode}{r.lastError ? `：${r.lastError}` : ''}</Typography.Text></div>}
                    </div>
                  ),
                },
                {
                ...
                { title: '最近成功', responsive: ['md'], render: (_v, r) => fmtTime(r.lastSuccessAt) },
                { title: '错误', ellipsis: true, responsive: ['md'], render: (_v, r) => (r.lastErrorCode ? <Typography.Text type="danger" title={r.lastError ?? ''}>{r.lastErrorCode}</Typography.Text> : '—') },
              ]}
            />
          </Card>
        </Col>
