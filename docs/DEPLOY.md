...
总览仪表盘顶部的“账号额度”显示 Claude Code 与 Codex **订阅账号**在服务商侧的额度已用比例与刷新时间（目前假定所有用户与服务器共用同一个账号，每个数据源各显示一份）。
...
- 平台每 10 分钟查询一次，页面上也可“立即查询”。查询来源自动选取该数据源下最近用量最大的采集目标（令牌最可能是新鲜的）；失败时（如 `TOKEN_EXPIRED`）自动换下一台。
...
- 采集脚本需 ≥ 1.5.0：自动安装的服务器会在首次查询时自动升级；手工安装（受限密钥）的服务器需重新拷贝 `remote/ccusage-collect.mjs`，并确认 `allowedDirs` 覆盖对应目录。
