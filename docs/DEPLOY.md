...
- “账目明细”从 2026-09 开始记账（页面右上角“汇率”里的“起始月份”可改）：更早的月份不进统计表和图，也不接受更早的账单。
...
没装 / 没在用的工具不会被反复查询：某个工具的目录在远端不存在（“未使用”）的目标不参加常规时点的采集，只在每天第一个批次里探测一次，装上后最迟第二天自动开始统计并回填；这类目标也不计入“数据未按期更新”。账号额度同理——没登录订阅账号的目标每 6 小时才再探测一次，登录已过期的账号每小时才再查一次，重新登录后自动恢复显示；需要立刻刷新时点仪表盘上的“立即查询”。
...
总览仪表盘顶部的“账号额度”显示 Claude Code 与 Codex **订阅账号**在服务商侧的额度已用比例与刷新时间。不同服务器可以登录不同账号：平台每轮先让各采集目标报告自己登录的账号（读取 CLI 本地保存的账号 ID 与邮箱，不联网），再按账号分组、每个账号只向服务商查询一次，页面按账号分别显示并列出使用它的服务器；某台服务器换了账号，下一轮（≤10 分钟）即反映。

- 查询由被采集服务器上的采集脚本就地完成：读取该账户下 CLI 自己保存的登录令牌（`~/.claude/.credentials.json`、`~/.codex/auth.json`），向服务商的用量接口发一次只读请求，只把“已用百分比、刷新时间、套餐名”回传给平台。**令牌不离开那台机器，不进平台数据库和日志；平台也不会替 CLI 续期令牌**（续期会让正在使用的 CLI 掉线）。
- 平台每 10 分钟查询一次，页面上也可“立即查询”。每个账号由该账号下最近用量最大的采集目标负责查询（令牌最可能是新鲜的）；失败时自动换同账号的下一台。
- `TOKEN_EXPIRED` 有两种：`token_expired` 表示那台服务器近期没用过该工具，令牌自然过期，下次使用时 CLI 会自动续期；`token_revoked` / `unauthorized` 表示登录被吊销，需要在那台服务器上重新登录。只有一台服务器在用、且令牌失效的账号查不到额度。
- 远端需要能访问服务商：采集脚本会依次从当前环境、登录 shell（`bash -lc env`）、`~/.claude/settings.json` 的 `env` 段里寻找代理设置。
- 这两个用量接口是 CLI 自己在用的内部接口，没有公开文档，服务商可能随时调整；结构变化时页面会显示明确的错误而不是错误数据。Codex 账号目前只返回周额度（没有 5 小时窗口）。
- **额度提醒**：在“告警规则 → 账号额度提醒”里启用后，任一账号的周额度剩余低于设定比例（默认 20%）时发邮件；同一窗口在一个刷新周期内只提醒一次，额度刷新后重新计。5 小时窗口默认不提醒（可勾选开启）。
- 用 API Key 而非订阅登录的账户没有这类额度，会显示 `NO_LOGIN`。
- 采集脚本需 ≥ 1.6.0：自动安装的服务器会在首次查询时自动升级；手工安装（受限密钥）的服务器需重新拷贝 `remote/ccusage-collect.mjs`，并确认 `allowedDirs` 覆盖对应目录。
